...
| コマンド | 内容 |
|---|---|
| `npm run dev` | 開発サーバー（ポート 5180・同じ Wi-Fi から見える） |
...
本番ビルドで見せたいときは `npm run build && npm run preview`（ポート 4180）。

**注意**

- Mac のサーバーが動いている間だけ使えます。Service Worker を入れていないのでオフラインでは開けません
- 初回アクセス時に macOS のファイアウォールが許可を求めることがあります
- 院内 Wi-Fi は端末どうしの通信を遮断している場合があります。つながらないときはテザリングか自宅の Wi-Fi で試してください
- サーバーはすべてのネットワークインターフェースで待ち受けます。止めるときは実行中のターミナルで Ctrl+C
